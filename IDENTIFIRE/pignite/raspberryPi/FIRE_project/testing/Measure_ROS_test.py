...
gray_folder = "/tmp/capture"
gray_files = sorted([f for f in os.listdir(gray_folder) if f.endswith(".gray")])
len_main = len(gray_files)

while True:
    new_files = sorted([f for f in os.listdir(gray_folder) if f.endswith(".gray")])
    if len(new_files) > len_main:
        len_main = len(new_files)
        gray_files = new_files
        ind = len_main - 1
        update(ind)
    else:
        print("no update")
