...
def main():
    setup()
    p = subprocess.Popen(["python3","Measure_ROS_test.py"], stdout = subprocess.PIPE, text=True)
    print(f"[INFO] waiting for all processes to start")
    time.sleep(5)
    name_base = 0
    while True:
        capture(1, name=f"test{name_base}")
        curr = os.listdir("/tmp/capture")[-1]
        curr = curr.split("_")[1].replace(".gray","")
        curr = int(curr)
        curr += 1
        name_base = curr
        line = p.stdout.readline()
        if line:
            print(f"data recieved {line.strip()}")
